...
class agente:
    def __init__(self, ventana):
        self.x = (150) #posicion en el eje x
        ...
        self.M=np.zeros((18, 18), dtype=int) # 0= unknow 1 = grown 2=wall (y,x) es el orden que usa numpy v = +y > = +x
...
def main():
    ...
    for i in range(dimX):
        for j in range(dimY):
            ...
            if (i == 30)and(j>=30 and j<= 140):
                estructura.append(wall(ventana, i, j))
            if (i == 140)and ((j>=30 and j<= 50) or (j>=90 and j<= 140)):
                estructura.append(wall(ventana, i, j))
            if (j == 30 )and (i>=30 and i<= 140):
                estructura.append(wall(ventana, i, j))
            if (j==140) and ((i>=30 and i<= 50) or (i>=90 and i<= 140)):
                estructura.append(wall(ventana, i, j))
